...
```bash
# x86_64 / amd64
sudo dpkg -i ./sfe_0.4.8_amd64.deb

# ARM64 / aarch64
sudo dpkg -i ./sfe_0.4.8_arm64.deb
```
...
从 v0.4.8 开始，`sfe --version` 和 `sfe -v` 由 shell 启动器直接读取版本号，不再启动 Python；安装包也会预编译编辑器核心 `.pyc` 并通过 `python -m sfe` 启动，以减少首次进入编辑界面的导入和编译成本。
