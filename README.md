...
```bash
# x86_64 / amd64
sudo dpkg -i ./sfe_0.6.0_amd64.deb

# ARM64 / aarch64
sudo dpkg -i ./sfe_0.6.0_arm64.deb
```
...
内置轻量 C 诊断会提示括号不匹配、字符串未闭合、重复 include 和明显缺少分号的行。状态栏会显示当前版本、诊断数量、buffer 位置、`DIRTY` 修改标记和 `RO` 只读标记。
