...
```bash
# x86_64 / amd64
sudo dpkg -i ./sfe_0.5.0_amd64.deb

# ARM64 / aarch64
sudo dpkg -i ./sfe_0.5.0_arm64.deb
```
...
项目文件树默认只显示顶层目录和文件，不会一次性展开整个项目。树面板获得焦点时，可以用 `j` / `k` 或方向键移动光标；选中目录按 `Enter` 展开，再按一次收起；选中文件按 `Enter` 在右侧编辑窗口打开或切换 buffer。`Ctrl-W` 在树面板和编辑窗口之间切换焦点，`q` 或 `Esc` 关闭树面板。
