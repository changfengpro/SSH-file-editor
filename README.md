...
- **Vim 风格操作：** 支持 Normal、Insert、Command 3 种模式，常用 `:w`、`:q`、`:wq`、`:q!` 命令可直接使用，并支持 `gg`、`G`、`Ctrl-D`、`Ctrl-U` 等常用移动。
...
```bash
# x86_64 / amd64
sudo dpkg -i ./sfe_0.8.0_amd64.deb

# ARM64 / aarch64
sudo dpkg -i ./sfe_0.8.0_arm64.deb
```
...
从 v0.5.4 开始，打开 ELF、可执行文件、包含 NUL 字节或非 UTF-8 的文件时，SFE 会显示“不支持该文件”的终端弹窗，并保持当前 buffer 不变，不再让 curses 因二进制内容崩溃。

从 v0.5.5 开始，`:make` 会在构建后刷新已显示的项目树，tree 面板获得焦点时按 `:` 也能进入命令模式，`:run` 在程序输出 stdout 或 stderr 时会打开 Run Output 列表显示输出内容。

从 v0.5.6 开始，`:make` 失败时会自动打开 Build Errors 列表；Run Output 中按 `r` 可重新运行上一次命令；并新增 Vim 风格的 `:42` 行跳转、`:pwd` 显示项目根目录、`:!cmd` 执行 shell 命令并显示输出。

从 v0.5.7 开始，Normal 模式新增 `gg`、`G`、`Ctrl-D` 和 `Ctrl-U`；`:q` 或退出快捷键遇到未保存 buffer 时会打开 Unsaved Buffers 列表，`:q!` 仍然强制退出；状态栏会显示 `DIRTY` 和 `RO` 标记，`:make` / `:run` 会显示耗时，例如 `Build OK in 0.32s`。

从 v0.6.0 开始，Normal 模式新增 `dd`、`yy`、`p`、`dw`、`cw`、`Ctrl-F` 和 `Ctrl-B`；命令模式新增 `:%s/old/new/g` 和 `:wqa`。`Ctrl-F` 改为 Vim 风格整屏下滚，搜索请使用 `/`。
从 v0.7.0 开始，`Ctrl-]` 在 `#include "local.h"` 上会优先跳转到本地头文件，系统头文件会给出可读提示；`:outline` 打开可跳转的当前文件符号列表，`:symbols` 兼容同一体验；C 诊断新增保守的重复局部变量、明显未使用局部变量、孤立 `else`、重复 `case` 和重复 `default` 检测。
从 v0.8.0 开始，`:files` 和 `Ctrl-P` 会打开可输入过滤的文件选择器，输入字符实时缩小匹配范围，`Backspace` 修改过滤词，`Enter` 打开当前项，`Esc` 关闭；项目树支持按字符快速定位可见项，刷新后尽量保留当前选中路径，状态栏会显示快速读取到的 Git 分支和文件选择器匹配数量。
...
```text
Ctrl-]       跳转到光标下符号的定义；在 #include 行上优先跳转本地头文件
Ctrl-O       返回跳转前的位置
:outline     查看当前文件符号列表，Enter 跳转到对应行
:symbols     :outline 的兼容别名
:diag        查看 C 诊断列表
:goto 42     跳转到第 42 行
:help        查看内置帮助
Ctrl-N       跳转到下一个诊断
...
内置轻量 C 诊断会提示括号不匹配、字符串未闭合、重复 include、明显缺少分号的行，以及保守识别出的重复局部变量、明显未使用局部变量、孤立 `else`、重复 `case` 和重复 `default`。状态栏会显示当前版本、诊断数量、buffer 位置、`DIRTY` 修改标记和 `RO` 只读标记。
...
:files              打开可过滤的项目文件列表，Enter 在新 buffer 中打开或切换文件
Ctrl-P              在 Normal 或 Insert 模式打开项目文件列表
:bind bn            按提示录入快捷键，之后用该快捷键执行 :bn
```

`:bd` 删除最后一个 clean buffer 后，会创建一个新的 `[No Name]` 空 buffer。`:files` / `Ctrl-P` 中可以直接输入过滤词，`Backspace` 删除过滤条件，`Esc` 关闭列表。项目树保持常驻左侧面板，选中文件按 `Enter` 会在编辑窗口打开或切换 buffer，并把焦点交回编辑窗口；tree 获得焦点时按字符会在当前可见项里快速定位。
...
:files               查看可过滤的项目文件列表，并按 Enter 打开选中文件
:recent              查看最近打开的项目文件
...
`:make` 和 `:run` 完成后，状态栏会显示耗时，例如 `Build OK in 0.32s` 或 `Run failed (1) in 0.08s`。
...
| --- | --- |
| `:w` | 保存当前文件 |
| `:q` | 退出；如果有未保存 buffer，会打开 Unsaved Buffers 列表 |
| `:q!` | 强制退出，不保存修改 |
| `:wq` | 保存并退出 |
| `:wqa` | 保存所有有文件名的 buffer 并退出 |
| `:x` | 保存并退出 |
...
| --- | --- |
| `Ctrl-O` / `F2` | 保存 |
| `Ctrl-Q` / `F10` | 退出；遇到未保存 buffer 时打开 Unsaved Buffers 列表 |
